...
module.exports=function exportBetterUtilBrowser(dep){
    ...
    //Rename the log
    const log = cX.log;
    log.rename('BetterUtil');

	
	const elemX=require('./browser/elements.util.js')({cX,log});
	const styleX=require('./browser/styling.util.js')({cX,log,elemX});
	const mobX=require('./browser/mobile.util.js')({cX,log,elemX});
	const restX=require('./browser/rest.util.js')({cX,log});
	
    //Combine everything onto the same object
    Object.assign(cX,elemX,styleX,mobX,restX);
